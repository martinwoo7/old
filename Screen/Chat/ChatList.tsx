...
const ChatList = ({ navigation }) => {
	...
	useEffect(() => {
		...
		const doSomething = async () => {
			...
			try {
				let lastFetchTimestamp = await AsyncStorage.getItem(
					"last_fetch"
				);
				if (lastFetchTimestamp === null) {
					...
					let temp =
						lastFetchTimestamp.seconds * 1000 +
						lastFetchTimestamp.nanoseconds / 1000000;
					...
					if (temp === null) {
						...
						if (firstFire.current) {
							...
							querySnapshot.forEach((doc) => {
								...
								const found = oldMessages.find(
									(item) => item.groupId === temp.groupId
								);
								if (found) {
							...
							oldMessages.sort(
								(a, b) =>
									b.modifiedAt.seconds - a.modifiedAt.seconds
							);
							...
							querySnapshot.docChanges().forEach((change) => {
								if (change.type === "added") {
									console.log(
										"New changes added in chatList: ",
										change.doc.data(),
										change.doc.id
									);
									const found = oldMessages.some(
										(item) => item.groupId === change.doc.id
									);
									...
									let temp = {
										...
										lastSender:
											change.doc.data().lastSender,
										name: change.doc.data().name,
										modifiedAt:
											change.doc.data().modifiedAt,
	...
	const handleLongpress = (item) => {
		// console.log(item)
		setData(item);
		Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light), handleVisible();
	};
			...
			<Pressable
				...
				onPress={() => {
					navigation.navigate("ChatScreen", {
						to: name,
						id: otherId,
						photoURL: profile_picture.photoURL,
						groups: item.groupId,
						type: item.type,
					});
				}}
			>
				<View style={styles.row}>
					...
					<Avatar
						...
						source={{ uri: profile_picture.photoURL }}
					/>
